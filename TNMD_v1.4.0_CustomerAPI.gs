 ...
 * Expected V2 storage:
 *   TNMD140IV2_INDEX
 *   TNMD140IV2_CHECKPOINT
 *
 * If the V2 implementation exposes its state through TNMD140IV2.index /
 * TNMD140IV2.checkpoint instead, the accessor fallbacks below support that.
 */
...
function tnmd140api_getIndex_() {
  if (typeof TNMD140IV2_INDEX !== 'undefined') {
    return TNMD140IV2_INDEX || {};
  }

  if (typeof TNMD140IV2 !== 'undefined' && TNMD140IV2 &&
      TNMD140IV2.index && typeof TNMD140IV2.index === 'object') {
    return TNMD140IV2.index;
  }
...
function tnmd140api_getCheckpoint_() {
  if (typeof TNMD140IV2_CHECKPOINT !== 'undefined') {
    return TNMD140IV2_CHECKPOINT || {};
  }

  if (typeof TNMD140IV2 !== 'undefined' && TNMD140IV2 &&
      TNMD140IV2.checkpoint && typeof TNMD140IV2.checkpoint === 'object') {
    return TNMD140IV2.checkpoint;
  }
...
/** Return all indexed customers, sorted by customer name. */
...
/** Return aggregate customer summary from the index only. */
...
/** Return summary for one category, defaulting to CABANG. */
...
function tnmd140api_testCustomerList() {
  return tnmd140api_envelope_('tnmd140api_testCustomerList', function() {
    ...
    return {
      ...
      valid: data.length === 14 && data.every(function(x) {
        return x.status === 'PASS' && x.complete === true && x.duplicate_count === 0;
      }),
      status: data.length === 14 && data.every(function(x) {
        return x.status === 'PASS' && x.complete === true && x.duplicate_count === 0;
      }) ? 'PASS' : 'FAIL'
    };
  });
